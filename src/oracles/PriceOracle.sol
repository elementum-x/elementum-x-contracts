...
contract PriceOracle is ChainlinkClient, ConfirmedOwner {
    ...
    function updateIndexPriceFromFeed() public onlyAuthorized {
        ...
        indexPrice = uint256(price);
        indexPriceTimestamp = updatedAt;
        emit PriceUpdated(PriceType.INDEX, indexPrice, indexPriceTimestamp);
    }
    ...
    function refreshIndexPrice() external onlyAuthorized {
        ...
        // 2. If stale, try to update from feed
        updateIndexPriceFromFeed();
        // 3. If still stale after feed update, fallback to direct request
        if (isIndexPriceStale()) {
            requestIndexPrice();
        }
    }
